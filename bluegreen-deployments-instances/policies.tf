...
resource "oci_identity_dynamic_group" "runcmddynamicgroup" {
  ...
  #matching_rule  = "Any {instance.id = '${oci_core_instance.compute_instance_blue.id}',instance.id = '${oci_core_instance.compute_instance_green.id}'}"
}
...
resource "oci_identity_policy" "devopspolicy" {
  ...
  statements = [
    ...
    # where request.instance.id='${oci_core_instance.compute_instance_blue.id}'",
